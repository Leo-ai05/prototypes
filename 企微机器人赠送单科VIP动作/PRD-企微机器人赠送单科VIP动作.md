...
分支：`feat/rule-action-subject-vip`（独立工作区 `.worktrees/subject-vip`，仅本地，未推送远程）

涉及页面：企业微信机器人 · 规则配置（新建规则弹窗与编辑规则弹窗的动作配置区、规则列表的动作类型列）

本地启动：在 `.worktrees/subject-vip/apps/telesale-web` 目录下执行 `pnpm install` 后 `pnpm dev`

本地地址：`http://localhost:8868/WH_CRM_v2/`（动作配置区独立预览页 `http://localhost:8868/WH_CRM_v2/preview-subject-vip.html`，该预览页为临时脚手架，未纳入提交范围）

关键态截图：`需求汇总/企微机器人赠送单科VIP动作/screenshots/`，共 6 张，分别为动作默认态、动作类型下拉含新选项、科目下拉清单、已选四科后其余科目置灰、赠送天数下拉、官方群发下动作类型置灰
