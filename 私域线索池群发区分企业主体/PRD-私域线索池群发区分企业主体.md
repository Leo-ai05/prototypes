...
1. 新建群发任务弹窗新增「企业主体」单选项，可选范围为当前销售已授权绑定且云设备在线的主体。
2. 企业微信标签选择区改为按所选主体加载该主体的标签，取消只读取固定单一主体标签的限制。
3. 线索分层人数、预估目标线索数、抽样预览均按所选主体过滤。
4. 群发任务提交时锁定所选主体作为任务快照，审批通过后由该主体的云设备发送。
5. 群发任务记录弹窗、群发任务详情弹窗、群发审核页、群发审批弹窗展示任务所属企业主体；任务记录弹窗新增企业主体筛选项。
6. 群发发起次数配额按「销售账号 + 企业主体」独立计数。
7. 单次目标线索数上限、近七天已群发排除策略均按所选主体独立判定。

### NON-GOAL（明确不做）

1. 不做一次任务跨两个主体合并发送。
2. 不做群聊群发。
3. 不改动私域线索分层的计算规则与标签同步链路。
4. 不改动企业微信账号授权流程与云设备登录流程。
5. 不允许对已提交的群发任务变更所属主体。
6. 不对本次上线前的历史群发任务回溯补齐主体信息。

### ACCEPT（可观察验收）

1. 销售仅有一个主体云设备在线时，打开新建群发任务弹窗，该主体自动选中且不可切换，其余流程可正常走完提交。
2. 销售两个主体云设备同时在线时，打开弹窗默认不选中主体，未选主体前分层人数、标签区、预估区均为待选态，提交审批按钮置灰。
3. 切换企业主体后，标签区重新加载为该主体的标签，已勾选标签被清空，分层人数与预估目标线索数重新计算。
4. 销售名下所有主体云设备均离线时，弹窗展示无可用主体提示，提交审批按钮置灰。
5. 同一销售在一个主体下发起一次群发后，在另一个主体下仍可发起一次，两个主体的配额互不占用。
6. 群发任务记录弹窗、任务详情弹窗、群发审核页均能看到任务所属企业主体，且任务记录弹窗可按企业主体筛选。
7. 审批通过后，消息仅发往所选主体下的好友，另一主体下的好友未被触达。
...
在线预览：本地 code-preview（不发测试环境）

原型模式：code-preview（在真实前端页面上叠加，非线框图）
代码仓库：`code/telesale-web_v2`
分支：`feat/private-lead-bulk-corp-split`（仅本地，默认不 push）
涉及页面：私域线索分层客户池页 → 新建群发任务弹窗、群发任务记录弹窗、群发任务详情弹窗；群发审核页 → 群发审批弹窗
本地启动：在 `code/telesale-web_v2/apps/telesale-web` 下执行 `pnpm dev`，访问 `http://localhost:5199/WH_CRM_v2`（需登录电销系统账号）
关键态截图：`需求汇总/私域线索池群发区分企业主体/screenshots/`

关键态截图清单：

| 序号 | 文件名 | 展示内容 |
|---|---|---|
| 1 | 01-双主体在线未选主体.png | 两个主体均在线且默认不选中，分层人数与预估为占位符，标签区提示先选主体，提交审批按钮置灰 |
| 2 | 02-已选葱粉园.png | 选中葱粉园后加载该主体标签组，分层人数与预估按该主体计算 |
| 3 | 03-切换到洋葱学园.png | 切换主体后标签组整体更换、已选标签清空、分层人数与预估重新计算，抽样预览展示所属企业主体 |
| 4 | 04-仅单主体在线自动锁定.png | 只有一个主体云设备在线时自动选中并标注仅此主体云设备在线 |
| 5 | 05-无在线主体拦截.png | 无在线主体时的橙色提示与提交审批按钮置灰 |
| 6 | 06-任务记录按主体筛选.png | 任务记录列表新增企业主体列与主体筛选，历史任务按葱粉园兜底展示 |
| 7 | 07-审批弹窗展示主体.png | 审批弹窗提交信息区与目标筛选条件区展示企业主体 |

> 截图由本地预览外壳挂载真实业务组件并配入模拟数据生成，界面与分支代码一致；外壳仅用于出图，不进入版本库。端到端联调仍需可登录的双主体测试账号（冒烟数据待产品提供）。
