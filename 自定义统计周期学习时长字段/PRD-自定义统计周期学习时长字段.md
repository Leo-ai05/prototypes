...
可交互原型（公网可访问）：https://leo-ai05.github.io/prototypes/%E8%87%AA%E5%AE%9A%E4%B9%89%E7%BB%9F%E8%AE%A1%E5%91%A8%E6%9C%9F%E5%AD%A6%E4%B9%A0%E6%97%B6%E9%95%BF%E5%AD%97%E6%AE%B5/

原型覆盖：会话属性列表（统计字段标识 + 只读不可编辑）、新建统计字段弹窗（六档周期单选、统计口径多选、名称后缀与全名实时预览、字段说明自动生成）、不可选与冲突变量的置灰与原因提示、组合重复与名称重复拦截、三种聚合方式（累计 / 新增 / 天数）的自动判定文案、删除二次确认与规则引用数、规则配置页引用统计字段。原型为交互与规则演示用，页面视觉与现行后台对齐，数值为示例数据。
