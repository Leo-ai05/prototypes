...
| 统计对象 | 运营为一个统计字段选定的那个会话属性，取值范围为全部数值型会话属性与「当日」口径的是否标记属性 |
| 窗口聚合方式 | 把统计对象在窗口内的表现折算成一个数的算法，由统计对象自身的口径决定，分累计、新增、日均、天数四种，运营不参与配置 |
...
| 日标记型 | 每日零点重置、取值为是或否的会话属性，如「当日是否有学习（布尔）」，窗口聚合方式为天数 |
...
| 3 | 会话属性页 — 统计对象选择面板 | 新增面板，按会话属性分类展示可作为统计对象的属性，支持搜索与单选，每项标注该属性将采用的窗口聚合方式 |
...
| 8 | 取值计算 | 新增按「统计窗口 + 统计对象」计算取值的逻辑，含累计、新增、日均、天数四种聚合算法 |
...
8. 改选「当日是否有学习（布尔）」，说明变为「按计天数统计：窗口内该标记为「是」的自然日天数」，名称变为「加微后0-7天学习天数」，即分阶段转人工常用的学习天数指标。
9. 重新打开面板并切到「活跃数据」分类，可看到统计对象不限于学情数据，「打开 APP 次数」「当日是否打开 APP」「累计活跃天数」同样可选。
10. 选中「打开 APP 次数（当日·次）」后把起始天改为 3、结束天改为 10，观察名称与说明实时刷新。
11. 把起始天改为 10、结束天改为 3，出现红字「结束天不能早于起始天」，「确定」按钮置灰。
12. 把结束天改为 45，出现红字「结束天需为 0 到 30 之间的整数」。
13. 把窗口改为 0 至 7 并选中「观看视频时长（当日·分钟）」，出现红字「该统计窗口与统计字段的组合已存在字段「加微后0-7天累计观看视频时长」，请勿重复创建」。
14. 改选「错题数（离线·日·个）」，出现红字提示生成的名称「加微后0-7天累计错题数」已被占用、该字段统计的是「错题数（实时·当日·个）」，提交仍被拦截。
15. 改选「看题型时长（当日·分钟）」，两条提示均消失、「确定」恢复可点，说明窗口相同但统计对象不同时允许创建。
16. 把窗口改为 8 至 14 后点击「确定」，弹窗关闭并提示创建成功，列表定位到「自定义字段」分类并回显新字段。
17. 在列表中点击「加微后0-7天累计观看视频时长」行的「编辑」，把结束天改为 14 并改选「错题数（实时·当日·个）」，弹窗顶部出现橙色影响提示。
18. 点击「确定」弹出二次确认弹窗，点「确认修改」后列表中的字段名称与描述同步更新。
19. 点击任一统计字段的「删除」，二次确认弹窗中展示「当前有 2 条规则正在使用该字段」。
20. 再次打开新建弹窗并修改任一输入项后点「取消」，弹出「填写内容尚未保存，确认关闭？」。
21. 顶部切到「规则配置」，点击触发条件行的变量选择框，「自定义字段」分类下可看到刚创建的统计字段，每项下方注明各自的统计口径。
22. 选中一个统计字段，条件行下方提示比较值单位与该字段统计对象一致；比较值输入 30.5，出现红字「请输入整数」。
23. 把操作符切换为「为空」，比较值输入框自动置灰不可填。

关键态截图：`需求汇总/自定义统计周期学习时长字段/proto-shots/`，共 27 张

洋葱内网链接（需飞连）：https://minio.yc345.tv/onionext/prototypes/custom-period-study-duration/index-20260907091653033539.html
...
| 统计字段 | 选择哪个会话属性作为本字段的统计对象 | 是 | 单选选择框 | 默认为空，占位文案「请选择会话属性」。点击后在下方展开会话属性选择面板（见 7.3）。选中后选择框展示该属性的完整名称。未选择时下方红字提示「请选择统计字段」 | 一个统计字段只对应一个会话属性，不做多选与组合运算。可选范围为全部数值型会话属性与「当日」口径的是否标记属性 |
| 聚合方式说明 | 告知运营本字段将以何种算法把统计对象折算成一个数 | — | 只读提示块 | 选中统计对象后在其下方展示蓝色提示块，内容为「本字段将按{聚合词}统计：{算法说明}，单位{单位}。聚合方式由该属性的口径决定（{判定依据}），不可手工更改」。未选中时不展示 | 聚合方式由系统按统计对象口径自动判定，运营既不需要也不能配置。四种聚合方式见 7.8 |
...
| 序号 | 统计对象口径 | 名称格式 | 示例 |
|---|---|---|---|
| 1 | 当日增量型 | 加微后{起始天}-{结束天}天累计{简名} | 观看视频时长（当日·分钟）→ 加微后0-7天累计观看视频时长 |
| 2 | 累计快照型 | 加微后{起始天}-{结束天}天新增{简名} | 观看视频时长（上线后累计·分钟）→ 加微后0-7天新增观看视频时长 |
| 3 | 比率型 | 加微后{起始天}-{结束天}天日均{简名} | 答题正确率（当日·%）→ 加微后0-7天日均答题正确率 |
| 4 | 日标记型 | 加微后{起始天}-{结束天}天{简名}天数 | 当日是否有学习（布尔）→ 加微后0-7天学习天数 |

前三种聚合词放在简名之前，日标记型的「天数」放在简名之后，因为「累计是否有学习」不成话，而「学习天数」才是运营口中的说法。

统计对象简名为该会话属性名称去掉口径括注后的部分，如「观看视频时长（当日·分钟）」的简名为「观看视频时长」。括注只说明原属性自身的周期与单位，放进统计字段名称里会与统计窗口的周期相互矛盾，所以不带入。日标记型属性的简名再去掉「当日是否」，如「当日是否有学习」的简名为「学习」、「当日是否打开 APP」的简名为「打开 APP」。
...
| 空态 | 分类下无可选属性时的兜底 | — | 文案 | 「自定义字段」分类下展示「自定义字段中暂无可作为统计对象的属性。统计字段本身不能再作为统计对象」；其余分类搜索无结果时展示「没有匹配的会话属性」 | — |
...
| 2 | **「当日」口径的是否标记属性**（布尔型）可选，如「当日是否有学习」「当日是否打开 APP」，按窗口内标记为「是」的天数统计 |
| 3 | 滚动窗口口径的布尔属性不可选，如「近7日是否活跃」。它在某天为真只表示过去 7 天内活跃过，不是当天独立的标记，数天数会重复计入同一次活跃 |
| 4 | 字符串型、日期型会话属性不可选，不在面板中展示 |
| 5 | 统计字段本身虽然是数值型，但不可被再次选作统计对象，避免嵌套定义 |
| 6 | 可选范围随会话属性的增减自动变化，新增可选属性后无需额外配置即可被选中 |
...
| 聚合方式 | 适用的统计对象口径 | 算法 | 判定依据 | 单位 |
|---|---|---|---|---|
| 累计 | 当日增量型：每日零点重置、只反映当天新增量的属性 | 窗口内各自然日的值相加 | 该属性是每日零点重置的当日增量值，逐日相加即窗口内总量 | 继承统计对象 |
| 新增 | 累计快照型：只增不减的历史累计属性 | 窗口结束当日的值，减去窗口起始天前一日的值 | 该属性是只增不减的累计快照值，直接相加会把同一份历史量重复计算多次 | 继承统计对象 |
| 日均 | 比率型：取值为百分比的属性 | 窗口内**有数据**的各自然日取平均，无数据的天不计入分母 | 该属性是比率值，跨天相加没有意义 | 继承统计对象 |
| 天数 | 日标记型：每日零点重置、取值为是或否的属性 | 窗口内该标记为「是」的自然日天数 | 该属性是每日重置的是否标记，数天数才有意义 | 天 |

天数聚合方式让运营可以直接拿到「加微后 0-7 天学习了几天」这类指标。分阶段转人工判定客户是否持续投入时，学习天数往往比学习时长更能反映粘性：一个客户在第 0 天集中学了 120 分钟然后再没打开过，和连续 7 天每天学 20 分钟，累计时长接近但意义完全不同。
...
| 4 | 可选统计对象 | 全部数值型会话属性与「当日」口径的是否标记属性，不限分类。统计字段本身不可被选作统计对象 |
| 5 | 统计内容 | 统计对象在统计区间内按 7.8 对应聚合方式折算出的数值 |
| 6 | 单位 | 继承统计对象的单位，如分钟、个、次、% |
| 7 | 取整 | 累计、新增、天数三种方式的结果取整数；日均方式的结果四舍五入到整数 |
...
| 2 | 客户在统计窗口内没有产生统计对象的任何数据（累计、新增或天数方式） | 取值为 0，不是空值，数值比较类条件按 0 参与判定 |
| 3 | 客户在统计窗口内没有产生统计对象的任何数据（日均方式） | 取值为**空**而不是 0。比率型属性没有数据时算不出平均值，0% 会被误读为表现极差；而累计、新增、天数三种方式的 0 就是「一分钟没学」「一天没学」的真实含义，与空值不冲突 |
