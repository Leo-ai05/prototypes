...
- **需求目标**：当运营进入某个规则组的规则列表时，系统应允许按分类查看、筛选和维护规则，且每条规则有且仅有一个分类。
- **核心逻辑简述**：当运营打开组内规则列表时，系统应展示分类、提供分类筛选和分类管理；当运营新建或编辑规则时，系统应要求选择一个分类后才能保存。
...
| 功能模块 | 调整内容 |
|---|---|
| 组内规则列表 | 增加分类列、分类筛选；可编辑时增加分类管理入口。 |
...
1. 点击分组进入规则列表，查看分类列并用分类筛选。
2. 点击分类管理，新增、改名；对仍有规则的分类尝试删除，查看阻断提示。
3. 点击新建规则，不选分类点保存，查看必填提示；选择分类后保存。
4. 点击复制，确认副本分类与原规则相同。
5. 点击切换只读权限，确认分类管理、新建和行内写操作不再展示。

关键态截图：`需求汇总/企微机器人规则分类管理/proto-shots/`
...
|---|---|---|---|---|---|
| 分类列 | 展示该规则当前所属分类。 | 列表字段 | 是 | 显示在规则名称之后；未改过分类的存量规则显示未分类。 | 只读权限同样可见。 |
| 分类筛选 | 按分类过滤当前组内规则。 | 筛选 | 否 | 选项为全部分类加「全部」；切换后只刷新当前组规则列表。 | 默认显示全部。 |
| 分类管理入口 | 打开分类管理弹窗。 | 按钮 | 否 | 仅可编辑权限展示，放在新建规则附近。 | 只读不展示。 |

异常与兜底：

1. 当前筛选分类下没有规则时，展示空列表提示「当前分类下没有规则」。
2. 分组列表页不增加分类列、不增加按分类筛选。
...
| 功能名称 | 功能说明 | 功能类型 | 是否必填 | 交互说明 | 备注 |
|---|---|---|---|---|---|
| 编辑权限 | 沿用现有规则配置编辑权限。 | 权限 | 是 | 无编辑权限时仍可看分类列和筛选，不可管理分类、不可改规则分类。 | 不新增权限点。 |
