...
| v1.1   | 2026-09-01 | 收窄管理页面范围：仅承载规则引擎触发的转人工原因，移除其余五类触发类型原因的展示，同步去掉触发类型筛选、触发类型列、原因分类列、是否可维护列 | AI PRD Writer |
...
运营先在「转人工原因管理」页面新增规则引擎的转人工原因，再回到规则配置页面为转人工动作选定原因；规则运行命中后，系统按该规则配置的原因发起转人工并写入转人工记录。

### SCOPE（本次必做）

1. 新增「转人工原因管理」一级页面，页面只承载规则引擎触发的转人工原因：列表查看、新增、重命名
...
4. 管理页面不展示规则引擎以外触发类型（用户被动、销售主动、标签回调、无法识别消息、空闲自动转回）的转人工原因，这些原因由系统写入，本次不新增、不修改、不提供查看入口
...
7. 管理页面列表中只出现规则引擎的转人工原因，页面无触发类型筛选项，列表中不出现用户被动、销售主动、标签回调、无法识别消息、空闲自动转回这五类原因
...
| 术语 | 定义 |
|------|------|
| 触发类型 | 转人工动作的发起源头分类，规则引擎是其中一类，转人工记录中仍会写入该分类 |
| 转人工原因 | 具体的业务场景名称，是转人工记录中可被聚合分析的最小分析维度 |
| 规则引擎原因 | 由规则引擎命中后发起转人工时所使用的转人工原因，是本次管理页面唯一承载、也是唯一可被运营新增和重命名的原因 |
| 系统默认原因 | 名称为「触发器动作」的规则引擎原因，是上线前所有转人工规则携带的原因，不可重命名 |
| 转人工动作 | 规则引擎动作类型之一，规则命中后把会话从 AI 托管切换到人工接待 |

## 四、调整范围

1. **转人工原因管理页面（本次新增）**：企微机器人后台新增一级菜单页面，只承载规则引擎转人工原因的列表查看、新增与重命名
...
可交互原型（公网，点击可直接操作）：[点击查看可交互原型](https://leo-ai05.github.io/prototypes/%E8%A7%84%E5%88%99%E8%BD%AC%E4%BA%BA%E5%B7%A5%E8%A7%A6%E5%8F%91%E5%8E%9F%E5%9B%A0%E7%BB%86%E5%8C%96/)

洋葱静态站（内网，需飞连）：[点击查看](https://minio.yc345.tv/onionext/prototypes/rule-transfer-reason-refine/index-20260901064649134012.html)

原型模式：html-wireframe（单文件可交互原型，无需登录、无需本地启动，非最终 UI）

涉及页面：转人工原因管理（新增一级页面）、规则配置（现有页面的规则弹窗）

可操作路径：
1. 默认进入「转人工原因管理」，列表中只有规则引擎的原因，仅「原因名称 / 创建时间 / 操作」三列，无触发类型筛选
2. 点击右上方「新增原因」，输入「触发器动作」点确定 → 触发重名校验红字提示
3. 改填「高意向信号」点确定 → 新增成功、列表新增行高亮
4. 点击该行「重命名」→ 打开重命名弹窗并回填原名称
5. 点顶部演示开关「当前：编辑权限」切到仅查看权限 → 新增按钮与重命名按钮置灰
6. 在搜索框输入不存在的名称 → 搜索无结果空态；点「列表：正常」切到空态 → 列表空态
7. 左侧切到「规则配置」→「新建规则」→ 展开转人工原因下拉，可看到刚新增的「高意向信号」
8. 不选原因直接保存 → 保存被拦截并红字提示
9. 点演示开关「原因选项：正常」「选项加载：成功」→ 分别演示无可选原因、选项加载失败
10. 关闭弹窗后点列表中存量规则的「编辑」→ 原因回显为「触发器动作」

关键态截图：`需求汇总/规则转人工触发原因细化/proto-shots/`，共 14 张

> 原型为可交互 HTML 页面，左侧导航包含两个视图：
> 1. 转人工原因管理：仅规则引擎的转人工原因列表、名称搜索、新增原因弹窗、重命名弹窗；顶部演示开关可切换「仅查看权限」「列表空态」「保存失败」三种状态
...
页面作为企微机器人后台的一级菜单，与「会话属性」平级，只承载规则引擎转人工原因的查看与维护。

**功能、弹窗、按钮类型**

| 功能按钮 | 功能说明 | 是否必填 | 功能类型 | 交互说明 | 备注 |
|---------|---------|---------|---------|---------|------|
| 菜单入口 | 从企微机器人后台左侧菜单进入本页面 | 不适用 | 菜单项 | 点击后进入转人工原因管理页面，默认展示规则引擎原因列表第一页 | 与「会话属性」同级，非规则配置页的子页 |
| 页面说明 | 告知运营本页面的维护边界 | 不适用 | 文字说明 | 页面标题下固定展示一行说明文案「本页面仅维护规则引擎触发的转人工原因，其余触发方式的原因由系统写入，不在本页面展示」 | 常驻展示，不可关闭 |
| 新增原因 | 打开新增转人工原因弹窗 | 不适用 | 按钮 | 位于列表右上方，常态可点击，点击后打开新增弹窗 | 无权限时按钮置灰，悬浮提示「暂无操作权限，请联系管理员」 |

### 7.2 转人工原因管理页面 - 原因列表

只展示规则引擎触发的转人工原因，不展示其余触发类型的原因。

**列表类型**

| 字段名称 | 字段说明 | 字段来源 | 备注 |
|---------|---------|---------|------|
| 原因名称 | 该原因的业务名称，是转人工记录展示与分析的最小维度 | 沿用现有原因字典中归属规则引擎的原因，本次新增的原因由运营维护 | 最长 20 个字符；系统默认原因「触发器动作」在名称后展示「系统默认」标识 |
| 创建时间 | 该原因加入字典的时间 | 沿用现有原因字典 | 精确到分钟；历史原因无记录时显示「—」 |
| 操作 | 行级操作区 | 本次新增 | 除系统默认原因外，每行展示「重命名」文字按钮；系统默认原因该列显示「—」 |
...
| 重置 | 清空搜索条件 | 不适用 | 按钮 | 点击后清空名称搜索并回到列表第一页 | 无 |
| 默认排序 | 列表默认展示顺序 | 不适用 | 排序规则 | 按创建时间正序，系统默认原因「触发器动作」排在最前 | 无 |
| 分页 | 翻页查看 | 不适用 | 分页器 | 每页 20 条，展示总条数 | 无 |
| 重命名 | 打开重命名弹窗 | 不适用 | 文字按钮 | 点击后打开重命名弹窗并回填当前名称 | 系统默认原因「触发器动作」不展示本按钮 |
...
| 搜索无结果 | 展示「没有符合条件的转人工原因」与「清空筛选」按钮 |
| 列表为空 | 展示「暂无转人工原因，点击右上方新增」与「新增原因」按钮 |

### 7.3 新增转人工原因弹窗

运营通过本弹窗新增规则引擎的转人工原因；本页面只维护规则引擎原因，弹窗内不再要求选择归属，新增结果一律归属规则引擎。
...
| 保存成功 | 关闭弹窗，顶部提示「新增成功」，列表清空搜索条件后刷新，新增行高亮 2 秒 |
...
| 管理页面新增或重命名完成 | 停留在管理页面，列表刷新；重命名保持当前搜索条件与分页，新增清空搜索条件回到第一页 |
