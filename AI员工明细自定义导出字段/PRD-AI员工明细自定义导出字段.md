...
可交互原型（公网，点击可直接操作）：[点击查看](https://leo-ai05.github.io/prototypes/AI%E5%91%98%E5%B7%A5%E6%98%8E%E7%BB%86%E8%87%AA%E5%AE%9A%E4%B9%89%E5%AF%BC%E5%87%BA%E5%AD%97%E6%AE%B5/)

原型模式：单文件可交互原型，无需登录、无需本地启动。

涉及页面：企业微信-AI员工明细页、「导出客户明细」弹窗、导出进度弹窗。

可操作路径：
1. 点击页面左上方「导出客户明细」打开弹窗，查看十二个默认选中的基础字段。
2. 选择时间范围并勾选会话变量，观察已选数量变化。
3. 点击顶部「25项上限」，查看未选字段置灰；取消任一已选项后可继续选择。
4. 点击「清空」，查看零字段校验提示与「确认导出」禁用状态。
5. 点击顶部「加载失败」，查看失败提示；点击「重新加载」恢复变量列表。
6. 选择近7天并确认导出，查看导出进度和完成反馈。

关键态截图：共六张，覆盖默认态、25项上限、加载失败、整页布局、零字段校验和导出进度。

代码侧验证（辅助材料，非交付原型）：已在本地前端与后端工作区按真实字段、交互和导出规则校正原型；未推送业务仓库，未发布测试环境。

洋葱内网链接：本次发布失败（上传连接被重置），不影响上述公网原型访问。
