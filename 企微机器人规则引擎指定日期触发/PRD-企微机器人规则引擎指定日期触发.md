...
1. 打开链接即为分组详情的规则列表默认态，可看到「国庆班课名额开放通知」展示未来触发时刻、「暑期结课日专属课包发放」展示触发时刻并带「已过期」标识。
2. 点击「＋ 新建规则」打开规则弹窗，默认触发器类型为「用户信息变更」。
3. 点击「触发器类型」下拉，可看到新增的第三项「指定日期触发」。
4. 选中「指定日期触发」，「触发时间」选择器出现，「官方群发」勾选框同时置灰；鼠标悬浮该勾选框可看到「指定日期触发暂不支持官方群发」提示。
5. 点击「触发时间」输入框展开日历面板，当天之前的日期为置灰不可点选态，可在面板顶部修改时刻后点「确定」。
...
9. 点击「暑期结课日专属课包发放」行的「编辑」，可看到触发时间置灰只读，悬浮提示「该规则的触发时间已过，如需重新发送请新建规则」。
10. 点击「国庆班课名额开放通知」行的「编辑」，可看到触发时间仍可修改为另一个未来时刻。
11. 先把触发器类型选为「定时任务」并勾选官方群发，再切换为「指定日期触发」，可看到勾选被自动取消并出现「已自动取消官方群发，指定日期触发不支持」提示。

关键态截图：`需求汇总/企微机器人规则引擎指定日期触发/proto-shots/`，共 13 张
